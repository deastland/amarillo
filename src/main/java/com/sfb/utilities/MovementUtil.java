...
public class MovementUtil {
	 ...
	 * 
	 * @param speedsMoved A map containing each impulse number that speed for a unit is set.
	...
	public static int totalHexesMoved(TreeMap<Integer, Integer> speedsMoved) {
		...
		// Get the speed for impulse 1 (should always be in the map) and create initial value.
	...
	/**
	 * Calculates the number of hexes moved, given a starting/ending impulse and a speed.
	 ...
	 * @return TRUE if the movement chart for @impulse shows movement for a speed of @speed
	 */
	public static boolean moveThisImpulse(int impulse, int speed) {
		
		int impulseChart[] = Constants.IMPULSE_CHART[impulse];
		for (int thisSpeed : impulseChart) {
			if (thisSpeed == speed) {
				return true;
			}
		}
